...
import { SerialPilotStream, OpenOptions } from '@serialpilot/stream'
import { OutputTranslator } from './output-translator'
import { autoDetect, AutoDetectTypes } from '@serialpilot/bindings-cpp'
...
const createPort = ({ path, baudRate }: { path: string, baudRate: number }) => {
  ...
  const port = new SerialPilotStream(openOptions)
