...
const port = new SerialPilot({ path: '/dev/my-great-device', baudRate: 9600 })
...
port.on('error', error => {
  if (error instanceof SerialPilotError && error.advice) {
    console.error(`${error.code}: ${error.message}`)
    console.error(`Advice: ${error.advice}`)
  } else {
...
port.write(largeMessage, error => {
  ...
  // When this runs data may still be buffered in the os or node serialpilot and not sent
  // out over the port yet. SerialPilot will wait until any pending writes have completed and then ask
