...
import { BindingInterface, OpenOptions, PortInfo, PortStatus, SetOptions, UpdateOptions } from '@serialpilot/bindings-interface'
...
const debug = debugFactory('serialpilot/bindings-cpp')
...
export class WindowsPortBinding implements BindingPortInterface {
  ...
  async write(buffer: Buffer): Promise<void> {
    ...
    const pendingWrite = (this.writeOperation ?? Promise.resolve()).catch(() => undefined)
    ...
    this.writeOperation = nextWrite.finally(() => {
      if (this.writeOperation === nextWrite) {
        this.writeOperation = null
      }
    })
