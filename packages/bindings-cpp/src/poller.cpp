...
Napi::Value Poller::destroy(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::HandleScope scope(env);
  // Release our strong persistent ref. NAPI will run ~Poller() once the JS
  // wrapper is GCed; the destructor closes the libuv handle via uv_close,
  // and onClose() frees poll_handle. Do NOT `delete this` here — the wrapper
  // is owned by NAPI and a manual delete would cause a double free.
  this->Reset();
  return env.Undefined();
}
