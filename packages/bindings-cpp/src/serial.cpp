...
inline BindingParity ToParityEnum(const Napi::String& napistr) {
  auto tmp = napistr.Utf8Value();
  const char* str = tmp.c_str();

  size_t count = strlen(str);
  BindingParity parity = BINDING_PARITY_NONE;
  if (!strncasecmp(str, "none", count)) {
    parity = BINDING_PARITY_NONE;
  } else if (!strncasecmp(str, "even", count)) {
    parity = BINDING_PARITY_EVEN;
  } else if (!strncasecmp(str, "mark", count)) {
    parity = BINDING_PARITY_MARK;
  } else if (!strncasecmp(str, "odd", count)) {
    parity = BINDING_PARITY_ODD;
  } else if (!strncasecmp(str, "space", count)) {
    parity = BINDING_PARITY_SPACE;
  }
  return parity;
}

 inline BindingStopBits ToStopBitEnum(double stopBits) {
  if (stopBits > 1.4 && stopBits < 1.6) {
    return BINDING_STOPBITS_ONE_FIVE;
  }
  if (stopBits == 2) {
    return BINDING_STOPBITS_TWO;
  }
  return BINDING_STOPBITS_ONE;
}

inline BindingRtsMode ToRtsModeEnum(const Napi::String& napistr) {
  auto tmp = napistr.Utf8Value();
  const char* str = tmp.c_str();

  size_t count = strlen(str);
  BindingRtsMode mode = BINDING_RTSMODE_HANDSHAKE;
  if (!strncasecmp(str, "enable", count)) {
    mode = BINDING_RTSMODE_ENABLE;
  } else if (!strncasecmp(str, "handshake", count)) {
    mode = BINDING_RTSMODE_HANDSHAKE;
  } else if (!strncasecmp(str, "toggle", count)) {
    mode = BINDING_RTSMODE_TOGGLE;
  }
  return mode;
}
...
NODE_API_MODULE(serialpilot, init);
