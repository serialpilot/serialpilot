...
import { SetOptions, BindingInterface, PortInterfaceFromBinding, OpenOptionsFromBinding, SerialPilotError, DisconnectedError, DisconnectInfo, PortBusyError, OpenFailedError, InvalidArgumentError } from '@serialpilot/bindings-interface'
const debug = debugFactory('serialpilot/stream')

export { SerialPilotError, DisconnectedError, DisconnectInfo } from '@serialpilot/bindings-interface'

export interface SerialPilotStreamEventMap {
  open: void
  data: Buffer
  close: DisconnectedError | null | undefined
  error: SerialPilotError | Error
...
export interface StreamOptions<T extends BindingInterface> {
  /**
   * The hardware access binding. `Bindings` are how SerialPilot talks to the underlying system. If you're using the `serialpilot` package, this defaults to `'@serialpilot/bindings-cpp'` which auto detects Windows (`WindowsBinding`), Linux (`LinuxBinding`) and OS X (`DarwinBinding`) and load the appropriate module for your system.
   */
  binding: T
...
export class SerialPilotStream<T extends BindingInterface = BindingInterface> extends Duplex {
  ...
  on(event: 'close', listener: (err?: DisconnectedError | null) => void): this
  on(event: 'error', listener: (err: SerialPilotError | Error) => void): this
  ...
  once(event: 'close', listener: (err?: DisconnectedError | null) => void): this
  once(event: 'error', listener: (err: SerialPilotError | Error) => void): this
  once(event: 'disconnect', listener: (info: DisconnectInfo) => void): this
  ...
  emit(event: 'close', err?: DisconnectedError | null): boolean
  emit(event: 'error', err: SerialPilotError | Error): boolean
  emit(event: 'disconnect', info: DisconnectInfo): boolean
  ...
  /**
   * Create a new serial port object for the `path`. In the case of invalid arguments or invalid options, when constructing a new SerialPilot it will throw an error. The port will open automatically by default, which is the equivalent of calling `port.open(openCallback)` in the next tick. You can disable this by setting the option `autoOpen` to `false`.
   * @emits open
   * @emits data
   * @emits close
   * @emits error
  ...
  constructor(options: OpenOptions<T>, openCallback?: ErrorCallback) {
    ...
    if (!settings.binding) {
      throw new InvalidArgumentError('"Bindings" is invalid pass it as `options.binding`', settings.path)
    }

    if (!settings.path) {
      throw new InvalidArgumentError(`"path" is not defined: ${settings.path}`, settings.path)
    }

    if (typeof settings.baudRate !== 'number') {
      throw new InvalidArgumentError(`"baudRate" must be a number: ${settings.baudRate}`, settings.path)
    }
  ...
  open(openCallback?: ErrorCallback): void {
    if (this.destroyed) {
      return this._asyncError(new OpenFailedError(this.path, this.baudRate), openCallback)
    }

    if (this.isOpen) {
      return this._asyncError(new PortBusyError(this.path, this.baudRate), openCallback)
    }
  ...
  * @param  {(string|array|buffer)} data Accepts a [`Buffer`](http://nodejs.org/api/buffer.html) object, or a type that is accepted by the `Buffer.from` method (e.g. an array of bytes or a string).
  ...
  _disconnected(err: Error) {
    ...
    const disconnectInfo: DisconnectInfo = { path: this.path, error: err }
    this.emit('disconnect', disconnectInfo)
    this.close(undefined, new DisconnectedError(err.message, this.path, this.baudRate))
  }
...
/**
 * Request a number of bytes from the SerialPilot. The `read()` method pulls some data out of the internal buffer and returns it. If no data is available to be read, null is returned. By default, the data is returned as a `Buffer` object unless an encoding has been specified using the `.setEncoding()` method.
 * @method SerialPilot.prototype.read
...
/**
 * Listening for the `data` event puts the port in flowing mode. Data is emitted as soon as it's received. Data is a `Buffer` object with a varying amount of data in it. The `readLine` parser converts the data into string lines. See the parsers section for more information on parsers, and the [Node.js stream documentation](https://nodejs.org/api/stream.html#stream_event_data) for more information on the data event.
 * @event data
 */
 ...
 * @method SerialPilot.prototype.pause
 * @see resume
 * @returns `this`
 */
 ...
 * @method SerialPilot.prototype.resume
 * @see pause
 * @returns `this`
 */
