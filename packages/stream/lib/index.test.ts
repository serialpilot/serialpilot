...
import { DisconnectedError, OpenOptions, SerialPilotStream } from './'
import { assert } from '../../../test/assert'

const sandbox = sinon.createSandbox()

describe('SerialPilot', () => {
  ...
  describe('constructor', () => {
    describe('autoOpen', () => {
      it('opens the port automatically', done => {
        new SerialPilotStream(openOpts, err => {
          assert.isNull(err)
          done()
        })
      })
      ...
      it('emits the open event', done => {
        const port = new SerialPilotStream(openOpts)
        port.on('open', done)
      })

      it("doesn't open if told not to", done => {
        const port = new SerialPilotStream({ ...openOpts, autoOpen: false })
        port.on('open', () => {
    ...
    it('passes the error to the callback when an bad port is provided', done => {
      new SerialPilotStream({ ...openOpts, path: '/bad/port' }, err => {
    ...
    it('emits an error when an bad port is provided', done => {
      const port = new SerialPilotStream({ ...openOpts, path: '/bad/port' })
      port.once('error', err => {
        assert.instanceOf(err, Error)
        done()
      })
    ...
    it('throws an error when bindings are missing', done => {
      try {
        new SerialPilotStream({ ...openOpts, binding: undefined } as any)
      } catch (err) {
        assert.instanceOf(err, Error)
        done()
      }
    ...
    it('throws an error when no port is provided', done => {
      try {
        new SerialPilotStream({ ...openOpts, path: '' })
      } catch (err) {
        assert.instanceOf(err, Error)
        done()
      }
    ...
    it('errors with a non number baudRate even with a callback', done => {
      try {
        new SerialPilotStream({ path: '/dev/exists', baudRate: 'whatever' } as any, () => {})
      } catch (err) {
        assert.instanceOf(err, Error)
        done()
      }
    ...
    it('throws an error when given bad baudrate option even with a callback', () => {
      assert.throws(() => {
        new SerialPilotStream({ path: 'foobar', baudrate: 9600 } as any, () => {})
      })
    })
    ...
    it('sets valid flow control individually', done => {
      ...
      const port = new SerialPilotStream(options)
  ...
  describe('property', () => {
    ...
    describe('.isOpen', () => {
      ...
      it('returns false when the port is opening', done => {
        const port = new SerialPilotStream<MockBindingInterface>({
          ...openOpts,
          autoOpen: false,
        })
  ...
  describe('instance method', () => {
    describe('#open', () => {
      it('passes the open options to the bindings without the stream stuff', done => {
        ...
        new SerialPilotStream(openOpts)
      })

      it('calls back an error when opening an invalid port', done => {
        const port = new SerialPilotStream({ ...openOpts, path: '/dev/unhappy', autoOpen: false })
        port.open(err => {
          assert.instanceOf(err, Error)
          done()
        })
      ...
      it('emits data after being reopened', done => {
        ...
        const port = new SerialPilotStream<MockBindingInterface>(openOpts, () => {
          port.close(() => {
            port.open(() => {
    ...
    describe('#close', () => {
      ...
      it('emits an "end" event for reading consumers when endOnClose is true', done => {
        const port = new SerialPilotStream({ ...openOpts, endOnClose: true })
        port.on('open', () => {
          port.on('end', () => {
            assert.isFalse(port.isOpen)
            done()
      ...
      it('errors when the port is not open', done => {
        const cb = function () {}
        const port = new SerialPilotStream({ ...openOpts, autoOpen: false }, cb)
        port.close(err => {
          assert.instanceOf(err, Error)
          done()
        })
    ...
    describe('#set', () => {
      it('errors when serialpilot not open', done => {
        const port = new SerialPilotStream({ ...openOpts, autoOpen: false })
        port.set({}, err => {
          assert.instanceOf(err, Error)
          done()
