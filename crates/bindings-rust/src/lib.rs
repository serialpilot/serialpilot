//! SerialPilot Rust is a cross-platform serial port library.
//!
//! The goal of this library is to expose a cross-platform and platform-specific API for enumerating
//! and using blocking I/O with serial ports.
//!
//! # Feature Overview
//!
//! The library has been organized such that there is a high-level `Port` trait that provides
//! a cross-platform API for accessing serial ports. This is the preferred method of interacting
//! with ports. The `Port::new().open*()` and `available_ports()` functions in the root
...
//! corresponding `TTYPort` and `COMPort` structs (that both implement the `Port` trait).
//! Using the platform-specific `Port::new().open*()` functions will return the
...
///
/// The `Err` type is hard-wired to [`serialpilot_rust::Error`](struct.Error.html).
...
///
/// [`clear`]: trait.Port.html#tymethod.clear
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
...
impl PortBuilder {
    ...
    pub fn path<'a>(mut self, path: impl Into<std::borrow::Cow<'a, str>>) -> Self {
        path.into().as_ref().clone_into(&mut self.path);
        self
    }
    ...
    pub fn open(self) -> Result<Box<dyn Port>> {
        #[cfg(unix)]
        return posix::TTYPort::open(&self).map(|p| Box::new(p) as Box<dyn Port>);

        #[cfg(windows)]
        return windows::COMPort::open(&self).map(|p| Box::new(p) as Box<dyn Port>);
...
/// for a new platform.
pub trait Port: Send + io::Read + io::Write {
...
/// Contains all possible USB information about a `Port`
...
/// Construct a builder of `Port` objects
///
/// `Port` objects are built using the Builder pattern through the `new` function. The
/// resultant `PortBuilder` object can be copied, reconfigured, and saved making working with
/// multiple serial ports a little easier.
///
/// To open a new serial port:
/// ```no_run
/// serialpilot_rust::new("/dev/ttyUSB0", 9600).open().expect("Failed to open port");
/// ```
pub fn new<'a>(path: impl Into<std::borrow::Cow<'a, str>>, baud_rate: u32) -> PortBuilder {
    PortBuilder {
