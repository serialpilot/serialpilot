...
#[cfg(all(target_os = "linux", not(target_env = "musl"), feature = "libudev"))]
fn port_type(d: &libudev::Device) -> Result<PortType> {
    match d.property_value("ID_BUS").and_then(OsStr::to_str) {
        Some("usb") => {
            ...
            Ok(PortType::UsbPort(UsbPortInfo {
        ...
        None if is_rfcomm(d) => Ok(PortType::BluetoothPort),
        None => find_usb_interface_from_parents(d.parent())
            .and_then(get_modalias_from_device)
            .as_deref()
            .and_then(parse_modalias)
            .map_or(Ok(PortType::Unknown), |port_info| {
                Ok(PortType::UsbPort(port_info))
            }),
...
fn port_type(service: io_object_t) -> PortType {
    let bluetooth_device_class_name = c"IOBluetoothSerialClient".as_ptr();
    let usb_device_class_name = c"IOUSBHostInterface".as_ptr();
...
cfg_if! {
    if #[cfg(any(target_os = "ios", target_os = "macos"))] {
        ...
        /// The `PortInfo` struct contains the name of the port which can be used for opening it.
        ...
        /// Scans the system for serial ports and returns a list of them.
        /// The `PortInfo` struct contains the name of the port
        ...
        fn read_port_type(path: &Path) -> Option<PortType> {
            let path = path
                .canonicalize()
                .ok()?;
            ...
            match subsystem.as_ref() {
                ...
                "amba" => Some(PortType::Unknown),
                "pci" => Some(PortType::PciPort),
                "pnp" => Some(PortType::Unknown),
                "serial-base" => Some(PortType::Unknown),
        ...
        fn usb_port_type(interface_path: &Path) -> Option<PortType> {
            let info = read_usb_port_info(interface_path)?;
            Some(PortType::UsbPort(info))
        }
        ...
        pub fn available_ports() -> Result<Vec<PortInfo>> {
            ...
            for path in sys_path.read_dir().expect("/sys/class/tty/ doesn't exist on this system") {
                ...
                let port_type = if is_rfcomm(&raw_path) {
                    ...
                    let Some(port_type) = read_port_type(&path) else {
                        continue;
                    };
                    port_type
                };
