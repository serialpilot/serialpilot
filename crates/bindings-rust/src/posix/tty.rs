...
use crate::{
    ClearBuffer, DataBits, Error, ErrorKind, FlowControl, Parity, Port, PortBuilder, Result,
    StopBits,
};
...
/// Instead, use the cross-platform `serialpilot_rust::new()`. Example:
///
/// ```no_run
/// let mut port = serialpilot_rust::new("/dev/ttyS0", 115200).open().expect("Unable to open");
/// # let _ = &mut port;
/// ```
///
...
///
/// ```no_run
/// use serialpilot_rust::{TTYPort, Port};
///
/// let (mut master, mut slave) = TTYPort::pair().expect("Unable to create ptty pair");
/// # let _ = &mut master;
/// # let _ = &mut slave;
/// // ... elsewhere
/// let mut port = TTYPort::open(&serialpilot_rust::new(slave.name().unwrap(), 0)).expect("Unable to open");
/// # let _ = &mut port;
/// ```
#[derive(Debug)]
pub struct TTYPort {
...
impl TTYPort {
    ...
    pub fn open(builder: &PortBuilder) -> Result<TTYPort> {
    ...
    /// Attempts to clone the `Port`. This allow you to write and read simultaneously from the
    ...
    ///
    /// Also, you must be very careful when changing the settings of a cloned `Port` : since
    ...
    ///
    /// This is the same as `Port::try_clone()` but returns the concrete type instead.
    ///
    /// # Errors
    ///
