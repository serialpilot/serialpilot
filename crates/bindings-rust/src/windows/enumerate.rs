...
use crate::{Error, ErrorKind, PortInfo, PortType, Result, UsbPortInfo};

const CONNECTOR_PUNCTUATION_SELECTION: &[char] = &[':', '_', '\u{ff3f}'];

/// Returns true if the given segment looks like a Windows-auto-generated device instance ID
/// rather than a real USB serial number.
///
/// Devices that don't expose a serial-number string descriptor get an instance ID Windows
/// composes from the parent USB hub: `<n>&<hex>&<port>&<index>` (typically four `&`-separated
/// segments where the first is a single hex digit and the rest are hex/digit components).
/// Real USB serial numbers are typically alphanumeric strings without ampersands.
fn is_auto_generated_instance_id(s: &str) -> bool {
    let parts: Vec<&str> = s.split('&').collect();
    if parts.len() < 3 {
        return false;
    }
    // Each segment must be hex digits or pure digits (Windows generates these from the
    // hub topology — never ASCII letters typical of vendor-supplied serial strings).
    parts
        .iter()
...
impl<'hwid> HwidMatches<'hwid> {
    fn new(hwid: &'hwid str) -> Option<Self> {
        ...
        //
        // Devices without a real serial number get an auto-generated Windows device-instance
        // ID of the form `<n>&<hex>&<port>&<index>` (e.g. `6&A694CA9&0&0000`). We must reject
        // these so we return `None` rather than the leading digit. Fixes upstream issue #203.
        let serial = if hwid_tail.starts_with('\\') || hwid_tail.starts_with('+') {
            hwid_tail.get(1..).and_then(|tail| {
                if is_auto_generated_instance_id(tail) {
                    return None;
                }
                let index = tail
                    .char_indices()
...
impl PortDevice {
    ...
    pub fn port_type(&mut self) -> PortType {
        self.instance_id()
...
mod tests {
    ...
    fn test_parsing_usb_port_information() {
        ...
        // Devices without a serial number return None rather than the leading digit
        // of the auto-generated Windows device-instance ID. Fixes upstream issue #203.
