...
/// Set all values that won't be affected by `PortBuilder` options.
pub(crate) fn init(dcb: &mut DCB) {
    ...
    // SerialPilot Rust doesn't support toggling DSR: so disable fOutxDsrFlow
    ...
    // null bytes won't be discarded by SerialPilot Rust
    dcb.set_fNull(false);
    // dcb.set_fRtsControl()
    // SerialPilot Rust does not handle the fAbortOnError behaviour, so we must make sure it's not enabled
