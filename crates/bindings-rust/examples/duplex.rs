...
//! a new file descriptor, and therefore a new `Port` object that's safe
...
fn main() {
    ...
    let port_name = &serialpilot_rust::available_ports().expect("No serial port")[0].port_name;
    let mut port = serialpilot_rust::new(port_name, 9600)
        .open()
