...
//!
//! This tool serves as a debugging aid when encountering errors using `SerialPilot Rust`. It should
...
use serialpilot_rust::{ClearBuffer, DataBits, FlowControl, Parity, Port, StopBits};

const TEST_MESSAGE: &[u8] = "Test Message".as_bytes();

fn main() {
    let matches = Command::new("SerialPilot Example - Hardware Check")
    ...
    let port2_name = matches
        .get_one::<String>("loopback-port")
        .map(String::as_str)
        .unwrap_or("");
    ...
    let mut port1 = match serialpilot_rust::new(port1_name, 9600).open() {
        Err(e) => {
    ...
    if !port2_name.is_empty() {
        ...
        let mut port2 = match serialpilot_rust::new(port2_name, 9600).open() {
            Err(e) => {
...
fn test_single_port(port: &mut dyn serialpilot_rust::Port, loopback: bool) {
    ...
    call_query_method_check!(port, Port::bytes_to_write);
    call_query_method_check!(port, Port::bytes_to_read);
...
fn test_dual_ports(port1: &mut dyn serialpilot_rust::Port, port2: &mut dyn serialpilot_rust::Port) {
    println!(
...
fn set_defaults(port: &mut dyn serialpilot_rust::Port) {
