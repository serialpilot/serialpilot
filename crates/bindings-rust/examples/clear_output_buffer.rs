...
use serialpilot_rust::ClearBuffer;
...
fn main() {
    ...
    let matches = Command::new("SerialPilot Example - Clear Output Buffer")
...
fn run(port_name: &str, baud_rate: u32, block_size: usize) -> Result<(), Box<dyn Error>> {
    let mut port = serialpilot_rust::new(port_name, baud_rate)
