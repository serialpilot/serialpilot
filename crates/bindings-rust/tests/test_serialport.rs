//! Tests for the `Port` trait.
mod config;

use config::{hw_config, HardwareConfig};
use rstest::rstest;
use serialpilot_rust::*;
...
fn test_opening_found_ports(hw_config: HardwareConfig) {
    ...
    let ports = serialpilot_rust::available_ports().unwrap();
...
fn test_opening_port(hw_config: HardwareConfig) {
    serialpilot_rust::new(hw_config.port_1, 9600)
        .open()
        .unwrap();
}
...
fn test_duplicating_port_config(hw_config: HardwareConfig) {
    let port1_config = serialpilot_rust::new(hw_config.port_1, 9600)
        .data_bits(DataBits::Five)
        .flow_control(FlowControl::None)
        .parity(Parity::None)
        .stop_bits(StopBits::One)
