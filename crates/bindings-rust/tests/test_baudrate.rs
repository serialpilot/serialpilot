...
use serialpilot_rust::{ClearBuffer, Port};
use std::ops::Range;
...
/// Test cases for setting the baud rate via [`PortBuilder`].
mod builder {
    use super::*;
    ...
    fn test_standard_baud_rate(hw_config: HardwareConfig, #[case] baud: u32) {
        let port = serialpilot_rust::new(hw_config.port_1, RESET_BAUD_RATE)
            .baud_rate(baud)
            .open()
            .unwrap();
    ...
    fn test_non_standard_baud_rate_fails_where_not_supported(
        ...
        let res = serialpilot_rust::new(hw_config.port_1, RESET_BAUD_RATE)
            .baud_rate(baud)
            .open();
        assert!(res.is_err());
    }
    ...
    fn test_transmit_at_standard_baud_rate(hw_config: HardwareConfig, #[case] baud: u32) {
        let mut port1 = serialpilot_rust::new(hw_config.port_1, RESET_BAUD_RATE)
            .baud_rate(baud)
            .timeout(TEST_MESSAGE_TIMEOUT)
            .open()
            .unwrap();
        let mut port2 = serialpilot_rust::new(hw_config.port_2, RESET_BAUD_RATE)
            .baud_rate(baud)
            .timeout(TEST_MESSAGE_TIMEOUT)
            .open()
            .unwrap();
...
/// Test cases for setting the baud rate via [`serialpilot_rust::new`].
mod new {
    use super::*;
    ...
    #[cfg_attr(not(feature = "hardware-tests"), ignore)]
    fn test_standard_baud_rate(hw_config: HardwareConfig, #[case] baud: u32) {
        let port = serialpilot_rust::new(hw_config.port_1, baud)
            .open()
            .unwrap();
        check_baud_rate(port.as_ref(), baud);
    }
    ...
    fn test_non_standard_baud_rate_fails_where_not_supported(
        ...
        assert!(serialpilot_rust::new(hw_config.port_1, baud)
            .open()
            .is_err());
    }
    ...
    #[cfg_attr(not(feature = "hardware-tests"), ignore)]
    fn test_transmit_at_standard_baud_rate(hw_config: HardwareConfig, #[case] baud: u32) {
        let mut port1 = serialpilot_rust::new(hw_config.port_1, baud)
            .timeout(TEST_MESSAGE_TIMEOUT)
            .open()
            .unwrap();
        let mut port2 = serialpilot_rust::new(hw_config.port_2, baud)
            .timeout(TEST_MESSAGE_TIMEOUT)
            .open()
            .unwrap();
...
/// Test cases for setting the baud rate via [`Port::set_baud_rate`].
mod set_baud {
    use super::*;
    ...
    #[cfg_attr(not(feature = "hardware-tests"), ignore)]
    fn test_standard_baud_rate(hw_config: HardwareConfig, #[case] baud: u32) {
        let mut port = serialpilot_rust::new(hw_config.port_1, RESET_BAUD_RATE)
            .open()
            .unwrap();
