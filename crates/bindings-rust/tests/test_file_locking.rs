...
use rstest::rstest;
use serialpilot_rust::ErrorKind;
use serialpilot_rust::Port;
...
mod checks {
    ...
    pub fn open_port_fails(hw_config: &HardwareConfig) {
        let port = serialpilot_rust::new(&hw_config.port_1, 115200).open();
    ...
    pub fn open_port_with_lock_mode_fails(hw_config: &HardwareConfig, lock_mode: LockMode) {
        let port = serialpilot_rust::new(&hw_config.port_1, 115200)
    ...
    #[must_use]
    pub fn open_port_successful(hw_config: &HardwareConfig) -> Box<dyn Port> {
        serialpilot_rust::new(&hw_config.port_1, 115200)
            .open()
            .unwrap()
    }
